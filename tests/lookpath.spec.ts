...
describe('lookpath', () => {
    ...
    const originalPath = process.env['Path'];
    // awaiting a synchronous function does nothing, which makes testing nice and easy
    for (const lookpath of [lookpathAsync, lookpathSync]) {
        beforeAll(async () => {
        ...
        it('should accept additional path by option', async () => {
            const withoutAdditionalPath = await lookpath('unique_binary_name');
            expect(withoutAdditionalPath).toBeUndefined();
            const additionalPath = path.join(__dirname, 'data', 'bin_3');
            const withAdditionalPath = await lookpath('unique_binary_name', { include: [additionalPath] });
